<script>
	...
	async function handleDerive() {
		...
		const derivedBuffer = await pbkdf2Hmac(cipher, site.toLowerCase(), 10000, 16);

		const indices = new Uint8Array(derivedBuffer).map((byte) => byte % allowedCharacters.length);
...
<main>
	...
	<article>
		...
		<input type="text" id="site" placeholder="GitHub" bind:value={site} oninput={handleDerive} />

		<button aria-label="Info" class="info" tabindex="-1">
			<i class="ri-information-line" title="Site is not case-sensitive. “GitHub” equals “github”."
			></i>
		</button>
		...
		>
			<i class={showCipher ? 'ri-eye-line' : 'ri-eye-off-line'} title="Show/hide cipher key"></i>
		</button>
		...
		>
			<i class={showDerivedPassword ? 'ri-eye-line' : 'ri-eye-off-line'} title="Show/hide password"
			></i>
		</button>

		<button disabled={!derivedPassword} onclick={handleCopy}>{copyButtonText}</button>
